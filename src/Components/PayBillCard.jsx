  ...
  toast('Bill paid successfully!')
    ...
    return (
        ...
        <span className='absolute bottom-0 -right-0 bg-base-300 p-1 rounded-full '>
            <img className='w-[50px] h-[50px] bg-base-200 rounded-full p-1' src={paybill.bill_type_icon} alt="" />
            </span>
            </div>
            <div className='p-5 space-y-2'>
                ...
                {
                    success && <p className='text-green-500 font-bold flex items-center justify-center
                    '><IoCheckmarkCircleOutline fill='green' size={20} />Payment Successfull</p>
                }
                <button onClick={() => handlePayBill(paybill.amount)} className='btn text-white bg-blue-700' disabled={isDisabled}>Pay Bill</button>
            </div>
